...
//  MAIN WRAPPER (Yellow background + padding)
...
// 🔥 Layout wrapper (sidebar + product grid)
...
const CategoryPage = () => {
    ...
    const getData = async () => {
      try {
        ...
        const { data } = await axios.get(
          `http://localhost:8000/products/category/${name}`
        );
  ...
  return (
    <Container>
      {/* BACK BUTTON */}
      <BackButton onClick={() => navigate(`/category/${name}`)}>
        ← Back to {name.toUpperCase()}
      </BackButton>

      {/* CATEGORY BANNER */}
      <CategoryBanner categoryName={name} />
